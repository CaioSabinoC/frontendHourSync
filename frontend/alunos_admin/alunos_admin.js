/* =====================================================
   HourSync — alunos_admin.js (INTEGRADO AO BACKEND)
...
function getStatus(aluno) {
  ...
  const horasExigidas  = aluno.cursoId?.horasExigidas || 0;
...
function renderTabelaAlunos() {
  ...
  const filtered = term
    ? alunosData.filter(a =>
        ...
        a.cursoId?.nome?.toLowerCase().includes(term))
    : [...alunosData];
  ...
  filtered.forEach(aluno => {
    ...
    const cursoNome     = aluno.cursoId?.nome || '—';
...
async function abrirModalAluno(aluno) {
  ...
  try {
    ...
    container.innerHTML = `
      <div class="mb-2"><strong>Curso:</strong> <span style="color:#6c83e6;">${aluno.cursoId?.nome || '—'}</span></div>
...
async function cadastrarAluno() {
  ...
  try {
    await apiFetch('/usuarios', {
      method: 'POST',
      body: JSON.stringify({ nome, matricula, email, senha, cursoId, role: 'ALUNO' })
    });
